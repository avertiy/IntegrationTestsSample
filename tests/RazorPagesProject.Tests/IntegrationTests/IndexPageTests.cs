...
namespace RazorPagesProject.Tests
{
        ...
        IClassFixture<CustomWebApplicationFactory<RazorPagesProject.Startup>>
    {
        ...
        [Fact]
        public async Task Count_Messages_On_Index_Page()
        {
            // Arrange
            var defaultPage = await _client.GetAsync("/");
            var content = await HtmlHelpers.GetDocumentAsync(defaultPage);
            var messagesInHtml = content.QuerySelectorAll("div.panel-body ul li");
            
            // Assert
            Assert.Equal(Utilities.GetSeedingMessages().Count, messagesInHtml.Length);
            foreach (var message in messagesInHtml)
            {
                Assert.NotNull(Utilities.GetSeedingMessages().Find((x) => message.InnerHtml.Contains(x.Text)));
            }
        }
